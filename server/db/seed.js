...
/**
 * DIS-1797: Galería de Hielo — la descripción promete interacción pero no indica cómo.
 * Agregar hints de examine para figuras y columnas, que ya tienen contenido.
 * También actualizar la pregunta retórica "¿Son los cadáveres?" por una narrativa con cierre.
 */
function migrateGaleriaHieloExamineDIS1797() {
  try {
    const room = db.getRoom(11);
    if (!room) {
      console.warn('[seed] migrateGaleriaHieloExamineDIS1797: sala 11 no existe — omitiendo.');
      return;
    }
    if (room.description && room.description.includes('examine figuras')) {
      console.log('[seed] migrateGaleriaHieloExamineDIS1797: ya migrado. ✓');
      return;
    }
    const newDesc = 'Columnas de hielo translúcido atrapan figuras deformadas en su interior. El aliento se congela al instante. Las paredes brillan con una luz azul espectral. Algunos de esos cuerpos congelados llevaban equipo —aventureros que llegaron hasta aquí y no volvieron. (Usá «examine figuras» o «examine columnas» para investigar.)';
    db.upsertRoom({ ...room, description: newDesc });
    db.persist();
    console.log('[seed] migrateGaleriaHieloExamineDIS1797: Sala 11 — descripción actualizada con hints de examine. DIS-1797 ✓');
  } catch (e) {
    console.warn('[seed] migrateGaleriaHieloExamineDIS1797:', e.message);
  }
}

/**
 * DIS-1811: Pícaro sin arma temática en fase temprana.
 * Agrega 'daga básica' al loot del Gnoll Merodeador (id=30) para que el Pícaro
 * pueda equipar un arma de clase en los primeros combates sin sacrificar el presupuesto de pociones.
 */
function migrateGnollDagaBasicaDIS1811() {
  try {
    const gnoll = db.getMonster(30);
    if (!gnoll) {
      console.log('[seed] migrateGnollDagaBasicaDIS1811: Gnoll Merodeador (id 30) no existe — omitiendo.');
      return;
    }
    const loot = Array.isArray(gnoll.loot) ? gnoll.loot : JSON.parse(gnoll.loot || '[]');
    if (loot.includes('daga básica')) {
      console.log('[seed] migrateGnollDagaBasicaDIS1811: daga básica ya está en el loot. ✓');
      return;
    }
    db.updateMonster(30, { loot: [...loot, 'daga básica'] });
    db.persist();
    console.log('[seed] migrateGnollDagaBasicaDIS1811: daga básica agregada al loot del Gnoll Merodeador (id 30). DIS-1811 ✓');
  } catch (e) {
    console.warn('[seed] migrateGnollDagaBasicaDIS1811:', e.message);
  }
}

/**
 * DIS-1812: Campeón Espectral del Coliseo sin contexto narrativo.
 * Agrega 'emblema del coliseo' al loot del Campeón Espectral (id=12).
 * El emblema es un ítem de lore que cuenta la historia de Vareth, el último campeón.
 * Tiene texto de examine enriquecido y puede venderse a Aldric por 35g.
 */
function migrateCampeonEmblemaDIS1812() {
  try {
    const campeon = db.getMonster(12);
    if (!campeon) {
      console.log('[seed] migrateCampeonEmblemaDIS1812: Campeón Espectral (id 12) no existe — omitiendo.');
      return;
    }
    const loot = Array.isArray(campeon.loot) ? campeon.loot : JSON.parse(campeon.loot || '[]');
    if (loot.includes('emblema del coliseo')) {
      console.log('[seed] migrateCampeonEmblemaDIS1812: emblema del coliseo ya está en el loot. ✓');
      return;
    }
    db.updateMonster(12, { loot: [...loot, 'emblema del coliseo'] });
    db.persist();
...
function migrateFactionMisionContratoSangrientoDIS1918() {
  try {
    const rawDb = db.raw();
    if (!rawDb) {
      console.warn('[seed] migrateFactionMisionContratoSangrientoDIS1918: db no inicializada — omitiendo.');
      return;
    }
    const stmt = rawDb.prepare("SELECT description_template FROM faction_mission_definitions WHERE id = 'fm_orden_caza_agresiva'");
    const found = stmt.step();
    const row = found ? stmt.getAsObject() : null;
    stmt.free();
    if (!row) {
      console.log('[seed] migrateFactionMisionContratoSangrientoDIS1918: misión fm_orden_caza_agresiva no encontrada — sin cambios. ✓');
      return;
    }
    if (row.description_template && row.description_template.includes('desde que te uniste a la Orden')) {
      console.log('[seed] migrateFactionMisionContratoSangrientoDIS1918: ya migrado. ✓');
      return;
    }
    const newTemplate = 'Los mejores guerreros de la Orden no se esconden detrás de escudos. Matá {target} criaturas en postura agresiva (desde que te uniste a la Orden).';
    rawDb.run(
      "UPDATE faction_mission_definitions SET description_template = ? WHERE id = 'fm_orden_caza_agresiva'",
      [newTemplate]
    );
    db.persist();
    console.log('[seed] migrateFactionMisionContratoSangrientoDIS1918: texto misión "El Contrato Sangriento" aclarado. DIS-1918 ✓');
  } catch (e) {
    console.warn('[seed] migrateFactionMisionContratoSangrientoDIS1918:', e.message);
  }
}
...
/**
 * BUG-2248: El Esqueleto Guerrero (id=2) tiene respawn_room_id=3 (Sala de los Ecos)
 * pero debería ser sala 4 (Cámara del Tesoro). La quest "Polvo al Polvo" dice que están
 * en la Cámara del Tesoro, y DIS-2242 también señala esta inconsistencia.
 * Fix: corregir respawn_room_id a 4 y mover el monstruo a sala 4 si actualmente está en sala 3
 * por el efecto del fix DIS-P02 (que lo colocó en la sala incorrecta al arrancar).
 */
function migrateEsqueletoRespawnRoomBUG2248() {
  try {
    const esqueleto = db.getMonster(2);
    if (!esqueleto) {
      console.warn('[seed] migrateEsqueletoRespawnRoomBUG2248: Esqueleto Guerrero (id 2) no encontrado — omitiendo.');
      return;
    }
    if (esqueleto.respawn_room_id === 4) {
      // Ya está correcto
      return;
    }
    const updates = { respawn_room_id: 4 };
    // Si el monstruo está actualmente en sala 3 (colocado por DIS-P02 erróneamente), moverlo a sala 4
    if (esqueleto.room_id === 3) {
      updates.room_id = 4;
      console.log('[seed] migrateEsqueletoRespawnRoomBUG2248: Esqueleto Guerrero movido de sala 3 → sala 4 (Cámara del Tesoro). BUG-2248 ✓');
    } else {
      console.log('[seed] migrateEsqueletoRespawnRoomBUG2248: respawn_room_id corregido 3 → 4 (Cámara del Tesoro). BUG-2248 ✓');
    }
    db.updateMonster(2, updates);
    db.persist();
  } catch (e) {
 ...
 * Sube max_mana de 42 → 50 para todos los jugadores mago existentes.
 * (Las nuevas cuentas ya usan el valor actualizado en classes.js)
 */
function migrateMagoManaDIS2257() {
  try {
    const players = db.getAllPlayers ? db.getAllPlayers() : [];
    let changed = 0;
    for (const player of players) {
      if (player.player_class === 'mago' && (player.max_mana || 0) < 50) {
        db.updatePlayer(player.id, { max_mana: 50, mana: 50 });
        changed++;
        console.log(`[seed] migrateMagoManaDIS2257: jugador ${player.id} (${player.name}) max_mana ${player.max_mana}→50. ✓`);
      }
    }
    if (changed === 0) {
      console.log('[seed] migrateMagoManaDIS2257: ningún mago necesita actualización. ✓');
    } else {
      db.persist();
      console.log(`[seed] migrateMagoManaDIS2257: ${changed} mago(s) actualizados. DIS-2257 ✓`);
    }
  } catch (e) {
    console.warn('[seed] migrateMagoManaDIS2257:', e.message);
  }
}
