...
async function main() {
  ...
  migrateGnollMerodeadorIMPL1761(); // IMPL-VV-1761a: Gnoll Merodeador (id 30) — monstruo variable para Variación Viva
  migrateZombieCaminanteIMPL1761(); // IMPL-VV-1761b: Zombie Caminante (id 31) — monstruo variable undead para Variación Viva
  migrateElementalFuegoIMPL1761();  // IMPL-VV-1761c: Elemental de Fuego (id 32) — monstruo variable elemental para Variación Viva
  migrateBatVampireDescDIS1775();   // DIS-1775: Murciélago Vampiro — descripción actualizada para reflejar mechá drain life
  migrateFuenteEternaDescDIS1778();  // DIS-1778: Fuente Eterna sala 18 — texto "agua vacía" → "agua demorada" (consistencia ficción/mecánica)
  migrateSubastaNorthHintDIS1789();  // DIS-1789: Casa de Subastas sala 17 — pista de ruta norte hacia Prisión Subterránea
  migrateConclaveExamineDIS1796();   // DIS-1796: El Ojo del Cónclave — reducir scale_per_level 1.0→0.5 (nivel 2: 7→6 examines)
  migrateGaleriaHieloExamineDIS1797(); // DIS-1797: Galería de Hielo — desc actualizada con hints de examine figuras/columnas
  migrateGnollDagaBasicaDIS1811();     // DIS-1811: daga básica en loot del Gnoll Merodeador para Pícaro en fase temprana
  migrateCampeonEmblemaDIS1812();       // DIS-1812: emblema del coliseo (lore) en loot del Campeón Espectral
  migrateQuestEdictoDIS1916();          // BUG-1916: renombrar "El Contrato de Élite" → "El Edicto del Filo" (evita confusión con misión semanal de élite)
  migrateFactionMisionContratoSangrientoDIS1918(); // DIS-1918: aclarar texto misión "El Contrato Sangriento" (el contador empieza al unirse)
  migrateDIS2182FixCorredorDuplicates();        // DIS-2182: quitar olor-al-mercader duplicado de descripción fija de sala 2
  migrateSubastaRetornoHubDIS2093();            // DIS-2093: hint de retorno al hub desde sala 17 (implementado en engine.js)
  migrateMonsterRebalanceDIS2183();             // DIS-2183: rebalanceo combate niveles 1-3 — Zombie/Espectro ATK↑, Esqueleto ATK/HP↑
  migrateEsqueletoRespawnRoomBUG2248();         // BUG-2248: Esqueleto Guerrero respawn_room_id 3→4 (Cámara del Tesoro)
  migrateMagoManaDIS2257();                      // DIS-2257: Mago max_mana 42→50 para economía de maná early game
